...
import numpy as np
from xicommon import const, dtypes
...
class IsotopeDetector(BaseFilter):
    ...
    def __init__(self, context, batch_peaks=1000):
    ...
    def process(self, spectrum):
        ...
        # empty spectra don't have isotope cluster
        if (new_spec.mz_values is None
                or len(new_spec.mz_values) == 0):
            # if there are no peaks, we return the spectrum as is
            new_spec.isotope_cluster = []
            new_spec.isotope_cluster_charge_values = np.array([], dtype=np.int32)
            new_spec.isotope_cluster_mz_values = np.array([], dtype=np.float64)
            new_spec.isotope_cluster_intensity_values = np.array([], dtype=np.float64)
            new_spec.isotope_cluster_peaks = np.array([], dtype=dtypes.peak_cluster)
            new_spec.peak_has_cluster = np.array([], dtype=bool)
            return new_spec
        ...
        # maybe better would be a sum of intensities
        new_spec.isotope_cluster_intensity_values = np.array([np.max(spectrum.int_values[c.peaks])
                                                              for c in accepted_cluster])
    ...
    def find_isotope_clusters_for_charge(self, output_spectrum, spectrum, charge=1):
        ...
        raw_clusters = []
        cluster_len = []
        after_last_idx = len(masses)
        first_idx = 0
        # go through the masses in batch_peaks to avoid memory issues with large spectra
        while first_idx < after_last_idx:
            next_batch_index = first_idx + self.batch_peaks
            if next_batch_index > after_last_idx:
                next_batch_index = after_last_idx
            # get the current batch of peaks
            last_batch_mz = masses[next_batch_index - 1]
            mono_peak_masses = masses[first_idx:next_batch_index]

            # calculate the maximum m/z value for the end of the cluster
            # this is the maximum m/z value for the last isotope peak in the cluster
            # (cluster_calc_size + 1) is used to ensure that we include the last
            max_end_cluster_mz = \
                (self.config.isotope_config.cluster_calc_size + 1) * const.C12C13_MASS_DIFF / charge

            batch_extend = np.searchsorted(masses[
                next_batch_index:],
                last_batch_mz + max_end_cluster_mz, side='left')
            batch_peaks = masses[first_idx:batch_extend + next_batch_index]
            ...
            expected_isotope_masses = self.isotope_masses(
                mono_peak_masses.reshape(1, -1), charge)[:, :, np.newaxis]
            ...
            all_matches = np.isclose(batch_peaks, expected_isotope_masses,
                                     rtol=2 * self.context.get_isotope_rtol(spectrum.source_path),
                                     atol=0)
            ...
            raw_clusters.extend([np.where(cluster)[0] + first_idx
                                 for cluster in np.any(all_matches, axis=0)])
            ...
            cluster_len.append(np.all(~all_matches.T, axis=0).argmax(axis=1))

            first_idx = next_batch_index

        # replace 0s with cluster_calc_size
        cluster_len = np.hstack(cluster_len)
        ...
        all_clusters = [self.filter_cluster_peaks(cluster, masses, charge, spectrum.source_path)[:l]
    ...
    def filter_cluster_peaks(self, cluster, masses, charge, source_path):
        """
        ...
        :param source_path: (str) path to spectrum's origin
        """
        ...
        matches = np.isclose(sorted_cluster, expected_isotopes,
                             rtol=2 * self.context.get_isotope_rtol(source_path),
                             atol=0)
    ...
    def isotope_masses(self, mass, charge):
        ...
        increments = np.arange(
            self.config.isotope_config.cluster_calc_size) * const.C12C13_MASS_DIFF / charge
