...
from xicommon.cython import copy_byte_ranges, unique_byte_ranges
import numpy as np


def test_unique_byte_ranges():
    ...
    src_indices = np.array([3, 1, 6, 7, 5, 5, 3], np.intp)

    # Start and end indices for each partial sequence
    starts = np.array([1, 0, 0, 2, 0, 0, 0], np.intp)
    ends = np.array([3, 1, 0, 2, 3, 2, 2], np.intp)

    # View of the source strings as a 2D byte array.
    src_bytes = src_strings.view(np.uint8).reshape(len(src_strings), -1)

    # Run in-place unique
    unique_bytes, inverse = unique_byte_ranges(src_bytes, src_indices, starts, ends)
    # Convert result to strings
    unique_strings = unique_bytes.reshape(-1).view(src_strings.dtype)

    # Generate the equivalent result by copying the ranges to a new array,
    # and using np.unique on the result viewed as a string array.
    ...
    expected_unique, expected_inverse = np.unique(range_strings, return_inverse=True)

    # Check results match
    np.testing.assert_array_equal(unique_strings, expected_unique)
