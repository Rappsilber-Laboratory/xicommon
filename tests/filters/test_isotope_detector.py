...
from xicommon.config import Config, IsotopeDetectorConfig
...
ctx = MockContext(Config(ms2_tol='10ppm', isotope_config=config))
...
def test_isotope_detector_return_new_spectrum():
    ...
    assert np.lib.array_utils.byte_bounds(new_spec.mz_values) \
        == np.lib.array_utils.byte_bounds(spectrum.mz_values)
    assert np.lib.array_utils.byte_bounds(new_spec.int_values) \
        == np.lib.array_utils.byte_bounds(spectrum.int_values)
...
def test_simple_long_cluster():
    ...
    spectrum = Spectrum({'mz': 100, 'charge': 1},
                        [100,
                         100 + 1 * const.C12C13_MASS_DIFF,
                         100 + 2 * const.C12C13_MASS_DIFF,
                         100 + 3 * const.C12C13_MASS_DIFF,
                         100 + 4 * const.C12C13_MASS_DIFF,
                         100 + 5 * const.C12C13_MASS_DIFF,
                         100 + 6 * const.C12C13_MASS_DIFF,
                         100 + 7 * const.C12C13_MASS_DIFF,
                         100 + 8 * const.C12C13_MASS_DIFF,
                         100 + 9 * const.C12C13_MASS_DIFF,
                         100 + 10 * const.C12C13_MASS_DIFF,
                         100 + 11 * const.C12C13_MASS_DIFF,
                         100 + 12 * const.C12C13_MASS_DIFF,
                         100 + 13 * const.C12C13_MASS_DIFF,
                         100 + 14 * const.C12C13_MASS_DIFF,
                         100 + 15 * const.C12C13_MASS_DIFF,
                         100 + 16 * const.C12C13_MASS_DIFF,
                         100 + 17 * const.C12C13_MASS_DIFF,
                         100 + 18 * const.C12C13_MASS_DIFF,
                         100 + 19 * const.C12C13_MASS_DIFF],
                        np.full(20, 100),
                        'test_scan')
    detected = IsotopeDetector(ctx).process(spectrum)
...
def test_simple_isotope_detector_with_tolerance():
    ...
    detected = IsotopeDetector(MockContext(Config(isotope_config=config))).process(spectrum)
    assert_array_equal(detected.isotope_cluster_mz_values, [100])
    assert_array_equal(detected.isotope_cluster_intensity_values, [30])
    assert_array_equal(detected.isotope_cluster_charge_values, [1])
    expected_cluster_peaks = np.array(
...
def test_overlapping_cluster():
    ...
    # make sure - that the detected isotopes stay the same with different batch_sizes for detection
    for batch_peaks in range(3, len(masses) + 1):
        # create a new detector with the same context but different batch size
        detector = IsotopeDetector(ctx, batch_peaks=batch_peaks)
        detected_batch = detector.process(spectrum)

        assert_array_equal(detected_batch.isotope_cluster_peaks, detected.isotope_cluster_peaks)
        assert_array_equal(detected_batch.isotope_cluster_charge_values,
                           detected.isotope_cluster_charge_values)
        assert_array_equal(detected_batch.isotope_cluster_mz_values,
                           detected.isotope_cluster_mz_values)
